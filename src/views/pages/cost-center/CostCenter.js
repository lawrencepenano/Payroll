...
import {
    ...
    import { getCostCenters, getCostCenter, storeCostCenter , updateCostCenter, deleteCostCenter , clearCostCenter } from '../../../actions/costCenter';
    import PropTypes from 'prop-types';

const CostCenter = ({costCenters, costCenter, totalSize, getCostCenters, getCostCenter, storeCostCenter,  updateCostCenter, deleteCostCenter, clearCostCenter }) => {
    ...
    // Handle Deleting Cost Center 
    const handleDeletingCostCenter = (id) =>{
        Swal.fire({
          ...
          if (result.value) {
            const deleteStatus = await deleteCostCenter(id)
            if (deleteStatus) {
              Swal.fire(
                'DELETED!',
    ...
    const onSubmit = () => {
        if(action == "create"){
          ...
          Swal.fire({
            ...
            if (result.value) {
              const storeStatus = await storeCostCenter(formData)
              if (storeStatus) {
                Swal.fire(
                  'CREATED!',
        ...
        if(action == "update"){
            ...
            Swal.fire({
              ...
              if (result.value) {
                const updateStatus = await updateCostCenter(formData)
                if (updateStatus) {
                  Swal.fire(
                    'UPDATED!',
      ...
      formatter: (cell, row) => {
        return (
          <Fragment>
                ...
                <Button
                onClick={() => handleDeletingCostCenter(cell)}
...
CostCenter.propTypes = {
    getCostCenters: PropTypes.func.isRequired,
    getCostCenter: PropTypes.func.isRequired,
    storeCostCenter: PropTypes.func.isRequired,
    updateCostCenter: PropTypes.func.isRequired,
    deleteCostCenter: PropTypes.func.isRequired,
    clearCostCenter: PropTypes.func.isRequired,
  ...
  const mapStateToProps = state => ({
    costCenters: state.costCenter.costCenters,
    costCenter: state.costCenter.costCenter,
    totalSize: state.costCenter.totalSize
  })
  
  export default connect(mapStateToProps, { getCostCenters, getCostCenter, storeCostCenter , updateCostCenter, deleteCostCenter, clearCostCenter } )(CostCenter);
